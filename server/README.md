...
- **평일 기준 통계 계산**:
  ...
  - 연간/주간/일간 통계는 평일 릴리즈만 계산
  - 개발팀의 실제 근무 패턴에 맞는 정확한 분석 제공

- **주말 릴리즈 별도 추적**:

  - `weekendStats` 객체로 주말 릴리즈 개별 관리
  - 주말 릴리즈 비율 및 통계 계산
  - CSV에 별도 섹션으로 주말 릴리즈 리포트

- **상세한 릴리즈 분류**:
  - 개별 릴리즈에 `is_weekend` 플래그 추가 (YES/NO)
  - 평일/주말 구분을 통한 명확한 데이터 분석
  - 릴리즈 패턴의 시각적 구분 가능
...
- **CSV 파일 구조 개선**:
  ...
  - 헤더에 평일/주말 릴리즈 수 및 비율 표시
  - `=== WEEKEND RELEASE STATISTICS ===` 섹션 추가
  - 모든 통계 섹션에 `(WEEKDAYS ONLY)` 명시

- **테스트 케이스 업데이트**:
  - 주말 제외 로직을 반영한 테스트 수정
  - ISO Week 경계 케이스 테스트 보완
  - 평일/주말 구분 검증 테스트 추가

#### Data Insights

- **분석 결과**: daangn 조직 릴리즈의 100%가 평일에 수행됨
- **워라밸 지표**: 주말 릴리즈 0건으로 건전한 근무 문화 확인
- **실제 업무 패턴**: 평일 기준 통계로 더 정확한 개발 생산성 측정
